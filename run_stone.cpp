...
Run_stone::Run_stone(LAMMPS *lmp) : Command(lmp) {}

/* ---------------------------------------------------------------------- */

void Run_stone::command(int narg, char **arg)
{
  if (narg < 1) error->all(FLERR,"Illegal run_stone command");

  if (domain->box_exist == 0)
    error->all(FLERR,"Run_stone command before simulation box is defined");

  // ignore run_stone command, if walltime limit was already reached
  ...
  if (nevery == 0) {
    ...
    timer->init();
    timer->barrier_start();
    printf("\nstone, in Run_stone::command, nsteps%d call update->integrate->run_stone\n",nsteps);
    update->integrate->run_stone(nsteps);
    timer->barrier_stop();
  ...
  printf("stone, it is at the end of Run_stone::command\n");
}
