...
FixStonePimd::FixStonePimd(LAMMPS *lmp, int narg, char **arg) : Fix(lmp, narg, arg)
{
  max_nsend = 0;
  tag_send = nullptr;
  buf_send = nullptr;
  ...
  stone_step_size = update->dt/100;
  stone_step_numb = 100;

//stone: fix 1 all pimd method pimd fmass 1.0 sp 2.0 temp 300.0 nhc 4 size 0.005 numb 100
//stone:     0   1    2      3    4     5   6
  for (int i = 3; i < narg - 1; i += 2) {
    if (strcmp(arg[i], "method") == 0) {
      ...
      stone_step_size = utils::numeric(FLERR, arg[i + 1], false, lmp);
      if (stone_step_size < 0) error->universe_all(FLERR, "Invalid pimd run step size for fix pimd");
    } else if (strcmp(arg[i], "numb") == 0) {
      stone_step_numb = utils::numeric(FLERR, arg[i + 1], false, lmp);
      if (stone_step_numb < 0) error->universe_all(FLERR, "Invalid pimd run step numb for fix pimd");
    } else
      error->universe_all(FLERR, fmt::format("Unknown keyword {} for fix pimd", arg[i]));
  } // end of loop for the command parse;
...
void FixStonePimd::setup(int vflag)
{
  if (universe->me == 0 && universe->uscreen)
...
void FixStonePimd::initial_integrate(int /*vflag*/)
{
  nhc_update_v();
  nhc_update_x();
}
...
void FixStonePimd::final_integrate()
{
  nhc_update_v();
}
...
void FixStonePimd::post_force(int /*flag*/)
{
...
void FixStonePimd::nhc_update_x()
{
  int n = atom->nlocal;
  double **x = atom->x;
  double **v = atom->v;
  ...
  for (int i = 0; i < n; i++) {
      if (atom->mask[i] & groupbit) { // stone Jan 30 2022
        x[i][0] += dtv * v[i][0];
        x[i][1] += dtv * v[i][1];
        x[i][2] += dtv * v[i][2];
      }
...
void FixStonePimd::nhc_update_v()
{
  int n = atom->nlocal;
  int *type = atom->type;
  double **v = atom->v;
...
void FixStonePimd::nmpimd_init()
{
  ...
  lam = (double *) memory->smalloc(sizeof(double) * np, "FixStonePimd::lam");
...
void FixStonePimd::nmpimd_fill(double **ptr)
{
  comm_ptr = ptr;
...
void FixStonePimd::nmpimd_transform(double **src, double **des, double *vector)
{
  int n = atom->nlocal;
  int m = 0;
...
void FixStonePimd::spring_force()
{
  spring_energy = 0.0;
...
void FixStonePimd::comm_exec(double **ptr)
{
  int nlocal = atom->nlocal;
  ...
  if (nlocal > max_nlocal) {
    ...
    buf_recv = (double *) memory->srealloc(buf_recv, size, "FixStonePimd:x_recv");

    for (int i = 0; i < np; i++)
      buf_beads[i] = (double *) memory->srealloc(buf_beads[i], size, "FixStonePimd:x_beads[i]");
  }
  ...
  for (int iplan = 0; iplan < size_plan; iplan++) {
    ...
    if (nsend > max_nsend) {
      max_nsend = nsend + 200;
      tag_send =
          (tagint *) memory->srealloc(tag_send, sizeof(tagint) * max_nsend, "FixStonePimd:tag_send");
      buf_send =
          (double *) memory->srealloc(buf_send, sizeof(double) * max_nsend * 3, "FixStonePimd:x_send");
    }
...
int FixStonePimd::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/)
{
  int i, j, m;
...
void FixStonePimd::unpack_forward_comm(int n, int first, double *buf)
{
  int i, m, last;
...
double FixStonePimd::memory_usage()
{
...
void FixStonePimd::grow_arrays(int nmax)
{
  if (nmax == 0) return;
  int count = nmax * 3;

  memory->grow(array_atom, nmax, size_peratom_cols, "FixStonePimd::array_atom");
  memory->grow(nhc_eta, count, nhc_nchain, "FixStonePimd::nh_eta");
  memory->grow(nhc_eta_dot, count, nhc_nchain + 1, "FixStonePimd::nh_eta_dot");
  memory->grow(nhc_eta_dotdot, count, nhc_nchain, "FixStonePimd::nh_eta_dotdot");
  memory->grow(nhc_eta_mass, count, nhc_nchain, "FixStonePimd::nh_eta_mass");
}

/* ---------------------------------------------------------------------- */

void FixStonePimd::copy_arrays(int i, int j, int /*delflag*/)
{
  int i_pos = i * 3;
  int j_pos = j * 3;
...
int FixStonePimd::pack_exchange(int i, double *buf)
{
  int offset = 0;
  int pos = i * 3;
...
int FixStonePimd::unpack_exchange(int nlocal, double *buf)
{
  int offset = 0;
  int pos = nlocal * 3;
...
int FixStonePimd::pack_restart(int i, double *buf)
{
  int offset = 0;
  int pos = i * 3;
...
void FixStonePimd::unpack_restart(int nlocal, int nth)
{
...
int FixStonePimd::maxsize_restart()
{
  return size_peratom_cols + 1;
}
...
int FixStonePimd::size_restart(int /*nlocal*/)
{
  return size_peratom_cols + 1;
}
...
double FixStonePimd::compute_vector(int n)
{
