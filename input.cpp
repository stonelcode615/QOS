...
void Input::file()
{
  ...
  printf("stone, in the Input::file\n");
  while (true) {
  ...
  printf("\nstone in Input::file(), after execute_command()\n");
}
...
int Input::execute_command()
{
  ...
  if (command_map->find(command) != command_map->end()) {
    printf("\nstone, in Input::execute_command, command_map %d command = %s\n",command_map->size(),command);
...
{
  printf("stone, in *Input::command_creator\n");
  return new T(lmp);
}
...
void Input::pair_style()
{
  ...
  printf("stone, in Input::pair_style(), call force->create_pair() with %s\n",arg[0]);
