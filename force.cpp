...
Force::Force(LAMMPS *lmp) : Pointers(lmp)
{
  ...
  printf("stone, in Force constructor call create_factories()\n");
  create_factories();
}
...
void _noopt Force::create_factories()
{
  ...
  printf("stone, in Force::create_factories\n");
...
void Force::create_pair(const std::string &style, int trysuffix)
{
  ...
  int sflag;
  printf("stone, in create_pair %s, call new_pair\n",style.c_str());
...
Pair *Force::new_pair(const std::string &style, int trysuffix, int &sflag)
{
  ...
  if (pair_map->find(style) != pair_map->end()) {
    ...
    printf("stone, in new_pair 2, style: %s\n",style.c_str());
    return pair_creator(lmp);
  }

  error->all(FLERR,utils::check_packages_for_style("pair",style.c_str(),lmp));
