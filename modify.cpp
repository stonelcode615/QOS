...
void Modify::initial_integrate(int vflag)
{
  ...
  int ind_fix_stone = -1;
  for (int i = 0; i < n_initial_integrate; i++) {
      char *fix_stone_style = fix[list_initial_integrate[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        //printf("stone,    in Modify::initial_integrate, ind_fix_stone      = %d\n",ind_fix_stone);
        fix[list_initial_integrate[ind_fix_stone]]->initial_integrate(vflag);
  }
  else {
      //printf("stone, in Modify::initial_integrate,    fix_run_stone      = %d\n",fix_run_stone);
      for (int i = 0; i < n_initial_integrate; i++) 
      if (i != ind_fix_stone) fix[list_initial_integrate[i]]->initial_integrate(vflag);
  }
}
...
void Modify::post_integrate()
{
  ...
  int ind_fix_stone = -1;
  for (int i = 0; i < n_post_integrate; i++) {
      char *fix_stone_style = fix[list_post_integrate[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        printf("stone,       Modify::post_integrate,    ind_fix_stone      = %d\n",ind_fix_stone);
        fix[list_post_integrate[ind_fix_stone]]->post_integrate();
  }
  else {
      printf("stone,   Modify::post_integrate,        fix_run_stone      = %d\n",fix_run_stone);
      for (int i = 0; i < n_post_integrate; i++)
        if (i != ind_fix_stone) fix[list_post_integrate[i]]->post_integrate();
  }
}
...
void Modify::pre_exchange()
{
  ...
  int ind_fix_stone = -1;
  printf("stone,    Modify::pre_exchange,      n_pre_exchange        = %d\n",n_pre_exchange);
  for (int i = 0; i < n_pre_exchange; i++) {
      char *fix_stone_style = fix[list_pre_exchange[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  printf("stone,    Modify::pre_exchange,      ind_fix_stone         = %d\n",ind_fix_stone);
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        fix[list_pre_exchange[ind_fix_stone]]->pre_exchange();
  }
  else {
     for (int i = 0; i < n_pre_exchange; i++)
     if (i != ind_fix_stone) fix[list_pre_exchange[i]]->pre_exchange();
  }
}
...
void Modify::pre_neighbor()
{
  ...
  int ind_fix_stone = -1;
  printf("stone,    Modify::pre_neighbor,      n_pre_neighbor        = %d\n",n_pre_neighbor);
  for (int i = 0; i < n_pre_neighbor; i++) {
      char *fix_stone_style = fix[list_pre_neighbor[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  printf("stone,    Modify::pre_neighbor,      ind_fix_stone         = %d\n",ind_fix_stone);
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        fix[list_pre_neighbor[ind_fix_stone]]->pre_neighbor();
  }
  else {
     for (int i = 0; i < n_pre_neighbor; i++)
     if (i != ind_fix_stone) fix[list_pre_neighbor[i]]->pre_neighbor();
  }
}
...
void Modify::post_neighbor()
{
  ...
  int ind_fix_stone = -1;
  printf("stone,    Modify::post_neighbor,     n_post_neighbor       = %d\n",n_post_neighbor);
  for (int i = 0; i < n_post_neighbor; i++) {
      char *fix_stone_style = fix[list_post_neighbor[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  printf("stone,    Modify::post_neighbor,     ind_fix_stone         = %d\n",ind_fix_stone);
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        fix[list_post_neighbor[ind_fix_stone]]->post_neighbor();
  }
  else {
     for (int i = 0; i < n_post_neighbor; i++)
     if (i != ind_fix_stone) fix[list_post_neighbor[i]]->post_neighbor();
  }
}
...
void Modify::pre_force(int vflag)
{
  ...
  int ind_fix_stone = -1;
  printf("stone,    Modify::pre_force,         n_pre_force           = %d\n",n_pre_force);
  for (int i = 0; i < n_pre_force; i++) {
      char *fix_stone_style = fix[list_pre_force[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  printf("stone,    Modify::pre_force,         ind_fix_stone         = %d\n",ind_fix_stone);
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        fix[list_pre_force[ind_fix_stone]]->pre_force(vflag);
  }
  else {
     for (int i = 0; i < n_pre_force; i++)
     if (i != ind_fix_stone) fix[list_pre_force[i]]->pre_force(vflag);
  }
}
/* ----------------------------------------------------------------------
...
void Modify::pre_reverse(int eflag, int vflag)
{
  ...
  int ind_fix_stone = -1;
  printf("stone,    Modify::pre_reverse,       n_pre_reverse         = %d\n",n_pre_reverse);
  for (int i = 0; i < n_pre_reverse; i++) {
      char *fix_stone_style = fix[list_pre_reverse[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  printf("stone,    Modify::pre_reverse,       ind_fix_stone         = %d\n",ind_fix_stone);
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        fix[list_pre_reverse[ind_fix_stone]]->pre_reverse(eflag,vflag);
  }
  else {
     for (int i = 0; i < n_pre_reverse; i++)
     if (i != ind_fix_stone) fix[list_pre_reverse[i]]->pre_reverse(eflag,vflag);
  }
}
...
void Modify::post_force(int vflag)
{
  ...
  int ind_fix_stone = -1;
  for (int i = 0; i < n_post_force; i++) {
      char *fix_stone_style = fix[list_post_force[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        //printf("stone,    in Modify::post_force,        ind_fix_stone      = %d\n",ind_fix_stone);
        fix[list_post_force[ind_fix_stone]]->post_force(vflag);
  }
  else {
      //printf("stone,    Modify::post_force,           fix_run_stone      = %d\n",fix_run_stone);
      for (int i = 0; i < n_post_force; i++)
      if (i != ind_fix_stone) fix[list_post_force[i]]->post_force(vflag);
  }
}
...
void Modify::final_integrate()
{
  ...
  int ind_fix_stone = -1;
  for (int i = 0; i < n_final_integrate; i++) {
      char *fix_stone_style = fix[list_final_integrate[i]]->style;
      int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
      if (stone_comp_result == 0) ind_fix_stone = i;
  }
  if (fix_run_stone != 0) {
      if (ind_fix_stone != -1)
        //printf("stone,    in Modify::final_integrate,   ind_fix_stone      = %d\n",ind_fix_stone);
        fix[list_final_integrate[ind_fix_stone]]->final_integrate();
  }
  else {
      //printf("stone, in Modify::final_integrate,      fix_run_stone      = %d\n",fix_run_stone);
      for (int i = 0; i < n_final_integrate; i++)
      if (i != ind_fix_stone) fix[list_final_integrate[i]]->final_integrate();
  }
}
...
void Modify::end_of_step()
{
  ...
  for (int i = 0; i < n_end_of_step; i++) {
    char *fix_stone_style = fix[list_end_of_step[i]]->style;
    int stone_comp_result = strcmp(fix_stone_style, "stonePimd");
    if (stone_comp_result == 0) ind_fix_stone = i;
  }
  ...
  if (fix_run_stone != 0) {
    if (ind_fix_stone != -1)
      //printf("stone,    in Modify::end_of_step,       ind_fix_stone      = %d\n",ind_fix_stone);
      fix[list_end_of_step[ind_fix_stone]]->end_of_step();
  }
  else {
    //printf("stone, in Modify::end_of_step,          fix_run_stone      = %d\n",fix_run_stone); 
    for (int i = 0; i < n_end_of_step; i++){
      if (i != ind_fix_stone) fix[list_end_of_step[i]]->end_of_step();
    }
  }
}
...
Fix *Modify::add_fix(int narg, char **arg, int trysuffix)
{
  ...
  if (strcmp(arg[2],"stonePimd") == 0){
    printf("stone, in Modify::add_fix, FOUND fix stonePimd\n");
    fix_stone = 1;
  }
  printf("stone, ifix = %d, %s, fix_stone = %d\n", ifix,arg[2],fix_stone);
  if (ifix < nfix) {
    newflag = 0;
